...
3. Stage exactly the session-touched files that show up as dirty or untracked (`git add <path>` per file). Skip any session-touched file that isn't actually dirty.
...
7. Run `git push`.
8. Confirm the push succeeded, report the branch/remote, and repeat the list of any files left unstaged from step 4 so nothing is lost track of.
